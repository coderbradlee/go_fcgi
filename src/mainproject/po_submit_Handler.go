...
func get_response(t *PoData) (string){
	...
	if check_err!=nil{
		...
		return `{"error_code":"`+err_no+`","error_msg":"`+check_err.Error()+`","data":{"bill_no":"`+t.Data.Purchase_order.Po_no+`","bill_type":"Purchase Order","receive_by":"`+received+`",   "company":"`+t.Data.Purchase_order.Company+`","receive_at":"`+t.Data.Request_time+`"},"reply_time":"`+time.Now().Format("2006-01-02 15:04:05")+`"}`
	}
	...
	if err!=nil{
		...
		return `{"error_code":"`+s+`","error_msg":"`+err.Error()+`","data":{"bill_no":"`+t.Data.Purchase_order.Po_no+`","bill_type":"Purchase Order","receive_by":"`+received+`",   "company":"`+t.Data.Purchase_order.Company+`","receive_at":"`+t.Data.Request_time+`"},"reply_time":"`+time.Now().Format("2006-01-02 15:04:05")+`"}`
	}
    ...
    if err_encode!=nil{
    	...
    	return `{"error_code":"`+error_json_encode+`","error_msg":"`+err_encode.Error()+`","data":{"bill_no":"`+t.Data.Purchase_order.Po_no+`","bill_type":"Purchase Order","receive_by":"`+received+`",   "company":"`+t.Data.Purchase_order.Company+`","receive_at":"`+t.Data.Request_time+`"},"reply_time":"`+time.Now().Format("2006-01-02 15:04:05")+`"}`
    }
