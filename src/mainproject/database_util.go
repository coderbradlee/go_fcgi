...
func get_payment_term_id_chan(payment_term_id_chan chan<- string,payment_term,company_id string) {
    ...
    if payment_term_id!=""{
        ...
        _, err := db.Exec(
        `INSERT INTO t_payment_term(company_id,payment_term_id,payment_type_id,payment_method_id,sort_no,billing_days,time_node,deadline_date_type,createAt,createBy,dr,data_version) 
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,company_id,payment_term_id,payment_type_id,payment_method_id,1,billing_days,0,9,time.Now().Format("2006-01-02 15:04:05"),"go_fcgi",0,1)
        if err!=nil{
...
func get_item_master_id_chan(item_master_id_chan chan<- string,item_no,product_name,product_code string){
    ...
    db.QueryRow("select item_master_id from t_item_master where item_basic_id=?",item_basic_id).Scan(&item_master_id)
